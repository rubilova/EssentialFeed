...
class LoadFeedFromCacheUseCaseTests: XCTestCase {
    ...
    func test_load_requestsChacheRetrieval() {
    ...
    func test_load_deliversCachedImagesOnLessThanSevenDaysOldCache() {
        ...
        let lessThanSevedDaysOldTimestamp = fixedCurrentDate.adding(days: -7).adding(seconds: 1)
        let (sut, store) = makeSUT(currentDate: {fixedCurrentDate})
        
        expect(sut, toCompleteWith: .success(feed.models), when: {
            store.completeRetrieval(with: feed.local, timestamp: lessThanSevedDaysOldTimestamp)
        })
    }
    ...
    func test_load_deliversNoImagesOnSevenDaysOldCache() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
        let lessThanSevedDaysOldTimestamp = fixedCurrentDate.adding(days: -7)
        let (sut, store) = makeSUT(currentDate: {fixedCurrentDate})
